...
class Generator
{
    ...
    static void GenerateManagedWeakRefs(GlobalInfo all)
    {
        ...
        string class_dir = Path.Combine("src", "managed");
    ...
    {
        //string base_dir = Environment.CurrentDirectory;
        string moon_dir = Path.Combine("src", "runtime");
    ...
    {
        string srcdir = Path.Combine("src", "runtime");
        ...
        string palwin32dir = Path.Combine(paldir, "win32");
        ...
        //all_files.AddRange (Directory.GetFiles (palwin32dir, "*.h"));
    ...
    {
        //string base_dir = Environment.CurrentDirectory;
        string dir = Path.Combine("src", "runtime");
    ...
    static void GenerateTypeStaticCpp(GlobalInfo all)
    {
        ...
        Helper.WriteAllText("src/runtime/type-generated.cpp", text.ToString());
    }

    static void GenerateTypeH(GlobalInfo all)
    {
        const string file = "src/runtime/type.h";
        StringBuilder text;
        string contents = File.ReadAllText(file + ".in");
    ...
    static void GenerateKindCs()
    {
        ...
        string realfile = "src/managed/System.Windows/Mono/Kind.cs".Replace('/', Path.DirectorySeparatorChar);
        Helper.WriteAllText(realfile, text.ToString());
    }

    static void GenerateValueH(GlobalInfo all)
    {
        const string file = "src/runtime/value.h";
        StringBuilder result = new StringBuilder();
    ...
    static void GeneratePInvokes(GlobalInfo all)
    {
        ...
        NativeMethods_cs = File.ReadAllText(Path.Combine("src/managed/System.Windows/Mono/NativeMethods.cs".Replace('/', Path.DirectorySeparatorChar)));
        ...
        text.AppendLine("}");
        Helper.WriteAllText(Path.Combine("src/managed/System.Windows/Mono/GeneratedPInvokes.cs".Replace('/', Path.DirectorySeparatorChar)), text.ToString());
    }
    ...
    static void WriteMethods(GlobalInfo all, string NativeMethods_cs, StringBuilder text, Func<MethodInfo, bool> func)
    {
        ...
        foreach (MethodInfo method in methods)
        {
            if (!func(method) || !method.Annotations.ContainsKey("GeneratePInvoke"))
                continue;
            WritePInvokeMethod(NativeMethods_cs, method, text, method.IsPluginMember ? "libmoonplugin" : "libmoon");
            text.AppendLine();
        }
    }
    ...
    static void WritePInvokeMethod(string NativeMethods_cs, MethodInfo method, StringBuilder text, string library)
    {
        ...
        foreach (ParameterInfo parameter in cmethod.Parameters)
        {
            ...
            if (parameter.ParameterType.Value == "char*")
            {
                generate_wrapper = true;
                marshal_strings = true;
            }
        }
        ...
        text.Append(name);
        text.Append("\"");
        text.Append(", CharSet=CharSet.Ansi");
        text.AppendLine(")]");
        ...
        if (generate_wrapper)
        {
            ...
            if (marshal_strings)
            {
                foreach (var param in cmethod.Parameters)
                {
                    if (param.ParameterType.Value == "char*")
                    {
                        text.Append(tabs);
                        text.AppendLine($"\tIntPtr __{param.Name} = Marshal.StringToCoTaskMemUTF8({param.Name});");
                    }
                }
            }
            ...
            if (marshal_strings)
            {
                foreach (var param in cmethod.Parameters)
                {
                    ...
                    {
                        text.Append(tabs);
                        text.AppendLine($"\tMarshal.FreeCoTaskMem(__{param.Name});");
                    }
                }
            }
            ...
            if (marshal_string_returntype)
            {
                ...
                text.Append(tabs);
                text.AppendLine("\tstring s = Marshal.PtrToStringUTF8 (result);\t// *copy* unmanaged string");
                text.Append(tabs);
