...
#define CAIRO_LINE_CAP_TRIANGLE CAIRO_LINE_CAP_ROUND
    ...
    Shape::GetTransformOrigin() {
        if (GetStretch() == StretchNone && GetVisualParent() && GetVisualParent()->Is(Type::CANVAS))
            return Point(0, 0);
...
#define IS_SIGNIFICANT(dx, x) (IS_ZERO(dx) && (fabs(dx) * x - x > 1.0))
    ...
    Shape::OnPropertyChanged(PropertyChangedEventArgs *args, MoonError *error) {
        ...
        else if (args->GetId() == Shape::StrokeDashCapProperty || args->GetId() == Shape::StrokeDashArrayProperty || args->GetId() == Shape::StrokeEndLineCapProperty || args->GetId() == Shape::StrokeLineJoinProperty || args->GetId() == Shape::StrokeMiterLimitProperty || args->GetId() == Shape::StrokeStartLineCapProperty) {
            InvalidateStrokeBounds();
        }
