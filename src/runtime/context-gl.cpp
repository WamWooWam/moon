...
#define GETPROCADDR(type, name) \
    ...
    GLContext::GLContext(GLSurface *surface) : Context(surface) {
        unsigned i, j;
...
#if !USE_EGL && !USE_CGL && !USE_WGL && !USE_SDL2_GL
    ...
    void GLContext::SetupVertexData() {
        int i;
    ...
    void GLContext::SetupVertexData(const double *matrix,
        double x,
        double y,
        double width,
        double height) {
    ...
    void GLContext::SetupTexCoordData() {
        int i;
    ...
    void GLContext::SetupTexCoordData(const double *matrix,
        double du,
        double dv) {
        GLContext::SetupTexCoordData();
    }

    void GLContext::SetupTexUnit(GLenum target, GLint texture) {
    ...
    int GLContext::PixelAlignment(int stride) {
        int alignment = 8;
    ...
    int GLContext::PixelRowLength(int stride,
        int width,
        int size) {
    ...
    void GLContext::BlitYV12(unsigned char *data[],
        int stride[]) {
        Context::Target *target = Top()->GetTarget();
        MoonSurface *ms;
        Rect r = target->GetData(&ms);
    ...
    GLuint GLContext::CreateShader(GLenum shaderType,
        GLsizei count,
        const GLchar **str) {
        GLuint shader;
        GLint status;
    ...
    GLuint GLContext::GetVertexShader() {
    ...
    const char *GLContext::ProgramPrecisionString() {
        return "";
    }

    GLuint GLContext::GetBlendProgram() {
        GString *s;
        GLuint fs;
    ...
    void GLContext::Paint(Color *color) {
    ...
    GLuint GLContext::GetProjectProgram(double opacity, unsigned yuv) {
    ...
    void GLContext::Paint(MoonSurface *src,
        const double *matrix,
        double alpha,
        double x,
        double y) {
    ...
    void GLContext::Project(MoonSurface *src,
        const double *matrix,
        double alpha,
        double x,
        double y) {
    ...
    GLuint GLContext::GetConvolveProgram(unsigned size) {
        GString *s;
        GLuint fs;
        unsigned i;
    ...
    void GLContext::Blur(MoonSurface *src,
        double radius,
        double x,
        double y) {
        GLSurface *surface = (GLSurface *)src;
    ...
    GLuint GLContext::GetDropShadowProgram(unsigned size) {
        GString *s;
        GLuint fs;
        unsigned i;
    ...
    void GLContext::DropShadow(MoonSurface *src,
        double dx,
        double dy,
        double radius,
        Color *color,
...
#define ERROR_IF(EXP)                         \
    ...
    const char *GLContext::WritemaskToType(const char *writemask) {
    ...
    GLuint GLContext::GetEffectProgram(PixelShader *ps) {
        GString *s;
        GLuint fs;
        GLuint program;
        gpointer data;
            ...
            for (unsigned k = 0; k < op.meta.nsrcparam; k++) {
                ...
                if (op.meta.ndstparam)
                    sprintf(src[k],
                        ...
                        srcmod[k],
                        srcreg[k],
                        reg.writemask & 0x1 ? swizzle_str[source[k].swizzle.x] : "",
                        reg.writemask & 0x2 ? swizzle_str[source[k].swizzle.y] : "",
                        reg.writemask & 0x4 ? swizzle_str[source[k].swizzle.z] : "",
                        reg.writemask & 0x8 ? swizzle_str[source[k].swizzle.w] : "",
                        srcmodend[k]);
            }
    ...
    void GLContext::ShaderEffect(MoonSurface *src,
        PixelShader *shader,
        Brush **sampler,
        int *sampler_mode,
        int n_sampler,
