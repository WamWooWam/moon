...
namespace Moonlight {
    ...
    Grid::OnCollectionChanged(Collection *col, CollectionChangedEventArgs *args) {
        if (PropertyHasValueNoAutoCreate(Grid::ColumnDefinitionsProperty, col) || PropertyHasValueNoAutoCreate(Grid::RowDefinitionsProperty, col)) {
            InvalidateMeasure();
        }
        else {
    ...
    Grid::OnCollectionItemChanged(Collection *col, DependencyObject *obj, PropertyChangedEventArgs *args) {
        if (PropertyHasValueNoAutoCreate(Panel::ChildrenProperty, col)) {
            if (args->GetId() == Grid::ColumnProperty || args->GetId() == Grid::RowProperty || args->GetId() == Grid::ColumnSpanProperty || args->GetId() == Grid::RowSpanProperty) {
                InvalidateMeasure();
            ...
            if (args->GetId() != ColumnDefinition::ActualWidthProperty && args->GetId() != RowDefinition::ActualHeightProperty) {
                InvalidateMeasure();
            }
            return;
        }
