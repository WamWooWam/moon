...
#define MC_IGNORABLE_NAMESPACE_URI "http://schemas.openxmlformats.org/markup-compatibility/2006"
        ...
        *v = NULL;
        ...
        case Type::DOUBLE: {
            ...
            if (is_nan || errno || endptr == s || *endptr) {
                if ((!prop_name || !strcmp(prop_name, "Width") || !strcmp(prop_name, "Height")) && (!g_ascii_strcasecmp(s, "Auto") || is_nan))
                    d = NAN;
                else
                    break;
            }
    ...
    XamlElementInstanceNative::XamlElementInstanceNative(XamlElementInfoNative *element_info, XamlParserInfo *parser_info, const char *name, ElementType type, bool create_item) : XamlElementInstance(element_info, name, type) {
    ...
    XamlElementInstanceValueType::XamlElementInstanceValueType(XamlElementInfoNative *element_info, XamlParserInfo *parser_info, const char *name, ElementType type) : XamlElementInstance(element_info, name, type) {
        this->element_info = element_info;
        this->parser_info = parser_info;
    }
    ...
    XamlElementInstanceEnum::XamlElementInstanceEnum(XamlElementInfoEnum *element_info, const char *name, ElementType type) : XamlElementInstance(element_info, name, type) {
    }
    ...
    XamlElementInstanceManaged::XamlElementInstanceManaged(XamlElementInfo *info, const char *name, ElementType type, Value *obj) : XamlElementInstance(info, name, type) {
    ...
    dependency_object_add_child(XamlParserInfo *p, XamlElementInstance *parent, XamlElementInstance *child, bool fail_if_no_prop) {
        ...
        if (parent->element_type == XamlElementInstance::PROPERTY) {
            ...
            if (owner) {
                ...
                if (!types->IsSubclassOf(prop_type, Type::DEPENDENCY_OBJECT_COLLECTION) && !types->IsSubclassOf(prop_type, Type::RESOURCE_DICTIONARY))
                    return;
