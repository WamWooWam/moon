...
namespace Moonlight {
...
#endif
        return eo->GetManagedHandle().ToIntPtr();
    }
    ...
    bool Deployment::Initialize(const char* platform_dir, bool create_root_domain) {
        if (initialized)
            return true;
        ...
        if (create_root_domain) {
            ...
            Deployment::desktop_deployment->Initialize();
    ...
    Deployment* Deployment::GetCurrent() {
        Deployment* deployment = (Deployment*)MoonThread::GetSpecific(tls_key);
    ...
    void Deployment::SetCurrent(Deployment* deployment) {
        SetCurrent(deployment, true);
    }

    void Deployment::SetCurrent(Deployment* deployment, bool domain) {
        MoonThread::SetSpecific(tls_key, deployment);
    }

    Deployment::Deployment() : DependencyObject(this, Type::DEPLOYMENT), current_app(this, CurrentApplicationWeakRef) {
...
#if 0
    ErrorEventArgs* Deployment::ManagedExceptionToErrorEventArgs(void* exc) {
        int errorCode = -1;
        char* message = NULL;
    ...
    void Deployment::RegisterICalls() {

        Runtime::callbacks->mono_register_icalls((const void*)event_object_get_managed_object, (const void*)dependency_object_get_value_icall);
...
#if 0
    void Deployment::ManagedExceptionToMoonError(void* exc, MoonError::ExceptionType type, MoonError* error) {
        int errorCode = -1;
        char* message = NULL;
    ...
    static void unref_kept_alive_object(gpointer obj) {
        ((EventObject*)obj)->unref_delayed();
    }

    void Deployment::SetKeepAlive(EventObject* object, bool value) {
    ...
    void Deployment::EnsureManagedPeer() {
        EnsureManagedPeer(this);
    }

    void Deployment::EnsureManagedPeer(EventObject* forObj) {
        ...
        if (ensure_managed_peer) {
            ...
            Runtime::callbacks->moon_ensure_managed_peer(forObj, kind);
        }
    }
    ...
    Deployment::FreeGCHandle(GCHandle gchandle) {
        ...
        Runtime::callbacks->mono_gchandle_free(gchandle.ToIntPtr());
    }
        
    GCHandle Deployment::CloneGCHandle(GCHandle gchandle) {
        if (!gchandle.IsAllocated())
            return GCHandle::Zero;

        return GCHandle(Runtime::callbacks->mono_gchandle_new(gchandle.ToIntPtr(), false));
    }

    GCHandle Deployment::CreateWeakGCHandle(void* mono_object) {
        if (mono_object == NULL)
            return GCHandle::Zero;

        return GCHandle(Runtime::callbacks->mono_gchandle_new_weakref(mono_object, false));
    }

    GCHandle Deployment::CreateGCHandle(void* mono_object) {
        if (mono_object == NULL)
            return GCHandle::Zero;

        return GCHandle(Runtime::callbacks->mono_gchandle_new(mono_object, false));
    }

    void* Deployment::GetGCHandleTarget(GCHandle handle) {
        //return Runtime::callbacks->mono_gchandle_get_target(handle.ToIntPtr());
        return handle.ToIntPtr();
    }

    void Deployment::DestroyManagedApplication(gpointer plugin_instance) {
        Deployment::SetCurrent(this);
        Runtime::callbacks->moon_destroy_managed_application(plugin_instance);
    }

    bool Deployment::InitializeAppDomain() {
        return InitializeAppDomain("System.Windows, Version=2.0.5.0, Culture=neutral, PublicKeyToken=7cec85d7bea7798e");
    }

    bool Deployment::InitializeAppDomain(const char* system_windows_fullname) {
        bool result = true;
    ...
    GCHandle Deployment::CreateManagedXamlLoader(gpointer plugin_instance, XamlLoader* native_loader, const Uri* resourceBase) {
        // MonoObject* loader;
        // MonoObject* exc = NULL;
        ...
        void* loader = Runtime::callbacks->moon_create_xaml_loader(native_loader, plugin_instance, surface, resource_base);
        if (loader != nullptr) {
            return GCHandle(loader);
        }
    ...
    Value* Deployment::MonoXamlParserCreateFromFile(const char* file, bool create_namescope, bool validate_templates, MoonError* error) {
        Value* v;
        Deployment::SetCurrent(this);

        int retVal = Runtime::callbacks->moon_xaml_parser_create_from_file(file, create_namescope, validate_templates, &v);
        if (retVal == 0) {
        ...
        MoonError::FillIn(error, MoonError::XAML_PARSE_EXCEPTION, retVal, "XAML parsing failed.");
        return NULL;
    }

    Value* Deployment::MonoXamlParserCreateFromString(const char* xaml, bool create_namescope, bool validate_templates, MoonError* error, DependencyObject* owner) {
        Value* v;
        Deployment::SetCurrent(this);

        int retVal = Runtime::callbacks->moon_xaml_parser_create_from_text(xaml, create_namescope, validate_templates, &v);
        if (retVal == 0) {
            Value* clone = new Value(*v);
            Value::FreeValue(v);
            return clone;
    ...
    Value* Deployment::MonoXamlParserHydrateFromString(const char* xaml, Value* obj, bool create_namescope, bool validate_templates, MoonError* error) {
        Value* v;
        Deployment::SetCurrent(this);

        int retVal = Runtime::callbacks->moon_xaml_parser_hydrate_from_text(xaml, create_namescope, validate_templates, obj, &v);
        if (retVal == 0) {
            Value* clone = new Value(*v);
            Value::FreeValue(v);
            return clone;
    ...
    bool Deployment::InitializeManagedDeployment(gpointer plugin_instance, const char* culture, const char* uiCulture) {
        ...
        if (GetXapFilename() != nullptr) {
            ret = Runtime::callbacks->moon_initialize_deployment_xap(plugin_instance, GetXapFilename(), culture, uiCulture);
        }
        else {
            ret = Runtime::callbacks->moon_initialize_deployment_xaml(culture, uiCulture);
        }
...
#if DEBUG
    void Deployment::AddSource(const Uri* uri, const char* filename) {
    ...
    const char* Deployment::InternString(const char* str) {
        if (str == NULL)
            return NULL;
...
#if EVENT_ARG_REUSE
    PropertyChangedEventArgs* Deployment::GetPropertyChangedEventArgs() {
    ...
    void Deployment::ReleasePropertyChangedEventArgs(PropertyChangedEventArgs* args) {
    ...
    bool Deployment::IsShuttingDown() {
        VERIFY_MAIN_THREAD;
        return is_shutting_down;
    }

    void Deployment::Dispose() {
        Surface* surface;
...
#if MONO_ENABLE_APP_DOMAIN_CONTROL
    bool Deployment::ShutdownManagedCallback(gpointer user_data) {
        return ((Deployment*)user_data)->ShutdownManaged();
    }

    bool Deployment::ShutdownManaged() {
    ...
    void Deployment::delete_loaded_closure(EventObject* eo, int event_id, int token, gpointer closure) {
        delete (LoadedClosure*)closure;
    }

    bool Deployment::match_loaded_closure(int token, EventHandler cb_handler, gpointer cb_data, gpointer data) {
    ...
    void Deployment::proxy_loaded_event(EventObject* sender, EventArgs* arg, gpointer closure) {
    ...
    void Deployment::add_loaded_handler(EventObject* obj, int token, gpointer closure) {
    ...
    void Deployment::remove_loaded_handler(EventObject* obj, int token, gpointer closure) {
        Deployment* deployment = (Deployment*)closure;
    ...
    void Deployment::EmitLoadedAsync() {
        if (GetSurface()->IsLoaded())
            EmitAsync(Deployment::LoadedEvent, NULL, true);
    }

    void Deployment::LayoutUpdated() {
        Emit(Deployment::LayoutUpdatedEvent);
    }

    FontManager* Deployment::GetFontManager() {
        return font_manager;
    }

    GlyphTypefaceCollection* Deployment::GetSystemTypefaces() {
        return font_manager->GetSystemGlyphTypefaces();
    }

    Application* Deployment::GetCurrentApplication() {
        return current_app;
    }

    void Deployment::SetCurrentApplication(Application* value) {
        current_app = value;
    }

    void Deployment::SetEnsureManagedPeerCallback(EnsureManagedPeerCallback callback) {
        this->ensure_managed_peer = callback;
    }

    void Deployment::UnregisterHttpRequest(HttpRequest* request) {
    ...
    void Deployment::AbortAllHttpRequests() {
        http_requests.Clear(true);
    }
    ...
    bool Deployment::RegisterMedia(EventObject* media) {
        bool result;
    ...
    void Deployment::UnregisterMedia(EventObject* media) {
        MediaNode* node = NULL;
    ...
    void Deployment::DisposeAllMedias() {
        List* list;
    ...
    bool Deployment::DrainUnrefs(gpointer context) {
    ...
    void Deployment::UnrefDelayed(EventObject* obj) {
        UnrefData* list;
        UnrefData* item;
    ...
    void Deployment::TrackObjectCreated(EventObject* obj) {
        InterlockedIncrement((unsigned int*)&objects_created);
    ...
    void Deployment::TrackObjectDestroyed(EventObject* obj) {
        InterlockedIncrement((unsigned int*)&objects_destroyed);
    ...
    bool Deployment::IsLoadedFromXap() {
        return is_loaded_from_xap;
    }

    void Deployment::SetIsLoadedFromXap(bool flag) {
        is_loaded_from_xap = flag;
    }

    void Deployment::SetInitialization(bool init) {
        if (!is_initializing) {
    ...
    void Deployment::SetXapLocation(const Uri* location) {
        if (is_shutting_down)
            return;
    ...
    const Uri* Deployment::GetXapLocation() {
        return xap_location;
    }

    const Uri* Deployment::GetSourceLocation(bool* is_xap) {
        const Uri* result;
        bool xap = false;
    ...
    void Deployment::SetXapFilename(const char* filename) {
        g_free(xap_filename);
        xap_filename = g_strdup(filename);
    }

    const char* Deployment::GetXapFilename() {
        return xap_filename;
    }

    void Deployment::TrackPath(char* path) {
        paths.Append(new StringNode(path));
    }

    gint32 Deployment::GetDeploymentCount() {
        return deployment_count;
    }

    char* Deployment::CanonicalizeFileName(const char* filename, bool is_xap_mode) {
        // MonoObject* exc = NULL;
        // MonoObject* result;
        ...
        char* r;
        if (Runtime::callbacks->moon_canonicalize_file_name(filename, is_xap_mode, &r)) {
    ...
    char* Deployment::CreateMediaLogXml(const char** names, const char** values) {
        // MonoObject* exc = NULL;
        // MonoObject* result;
        // MonoClass* mono_helper;
        ...
        char* r;
        if (Runtime::callbacks->moon_create_media_log_xml(names, values, &r)) {
            //LOG_DEPLOYMENT("Deployment::CanonicalizeFileName (%s, %i) => %s\n", filename, is_xap_mode, r);
            return r;
        }
    ...
    void Deployment::SetUriFunctions(const UriFunctions* value) {
        memcpy(&uri_functions, value, sizeof(UriFunctions));
    }

    bool Deployment::VerifyDownload(const char* filename) {
        ...
        return Runtime::callbacks->moon_check_file_integrity(filename);
    }
...
void MonoProfiler::DumpJittedMethods() {
    g_hash_table_foreach(jitted_methods, dump_jitted_methods, NULL);
}

void MonoProfiler::method_jitted(MonoProfiler* prof, MonoMethod* method, MonoJitInfo* jinfo, int result) {
...
void MonoProfiler::profiler_shutdown(MonoProfiler* prof) {
}

void MonoProfiler::track_gchandle(MonoProfiler* prof, int op, int type, uintptr_t handle, MonoObject* obj) {
...
void accumulate_g_ptr_array_by_type(gpointer data, gpointer user_data) {
    GHashTable* by_type = (GHashTable*)user_data;
...
void MonoProfiler::DumpStrongGCHandles() {
    GHashTable* by_type = g_hash_table_new(g_str_hash, g_str_equal);
    GPtrArray* top_n_by_type = g_ptr_array_new();
...
void MonoProfiler::DumpTracesByType() {
    if (!type_name)
        return;
