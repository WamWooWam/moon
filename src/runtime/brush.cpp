...
namespace Moonlight {
    ...
    brush_matrix_invert(cairo_matrix_t *matrix) {
        ...
        if (status != CAIRO_STATUS_SUCCESS) {
            printf("Moonlight: Error inverting matrix { %.2f %.2f, %.2f %.2f, %.2f %.2f } falling back\n",
    ...
    ImageBrush::Dispose() {
        ...
        if (source) {
            BitmapImage *bitmap = (BitmapImage *)source->AsImageSource();
            if (bitmap)
                bitmap->RemoveAllHandlers(this);
        }
        TileBrush::Dispose();
    }
