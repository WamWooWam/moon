...
#elif PAL_WIN32_WINDOWING
#include "pal/win32/pal-win32.h"
#elif PAL_SDL2_WINDOWING
...
#define NO_EVENT_ID -1
    ...
    RuntimeManagedCallbacks *Runtime::callbacks = nullptr;

    void Runtime::InitializeCallbacks(RuntimeManagedCallbacks *callbacks) {
...
#if defined(USE_GLX) || defined(USE_WGL) || defined(USE_EGL) || defined(USE_SDL2_GL)
...
#define RUNTIME_INIT_BROWSER (RuntimeInitFlag)(RUNTIME_INIT_OCCLUSION_CULLING | RUNTIME_INIT_USE_UPDATE_POSITION | RUNTIME_INIT_USE_SHAPE_CACHE | RUNTIME_INIT_ALLOW_WINDOWLESS | RUNTIME_INIT_USE_IDLE_HINT | RUNTIME_INIT_ENABLE_MS_CODECS | RUNTIME_INIT_CREATE_ROOT_DOMAIN | GLX_RUNTIME_INIT | RUNTIME_INIT_ENABLE_TOGGLEREFS | RUNTIME_INIT_AUDIO_ALSA | RUNTIME_INIT_AUDIO_PULSE | RUNTIME_INIT_AUDIO_OPENSLES)
    ...
    Surface::Attach(UIElement *element) {
        ...
        // if (first && runtime_version && !strncmp("4.", runtime_version, 2)) {
...
dump_render_list (List *render_list)
{
    ...
    Surface::Resize(int width, int height) {
        if (width == normal_window->GetWidth() && height == normal_window->GetHeight())
            return;
...
// FIXME: Should we really be depending on GDK here, or should locking be pal-ized?
    ...
    Surface::CreateArgsForEvent(int event_id, MoonEvent *event) {
        if (event_id == UIElement::InvalidatedEvent || event_id == UIElement::GotFocusEvent || event_id == UIElement::LostFocusEvent)
            return MoonUnmanagedFactory::CreateRoutedEventArgs();
        else if (event_id == UIElement::MouseLeaveEvent || event_id == UIElement::MouseMoveEvent || event_id == UIElement::MouseEnterEvent)
            return new MouseEventArgs((MoonMouseEvent *)event);
        else if (event_id == UIElement::MouseLeftButtonMultiClickEvent || event_id == UIElement::MouseLeftButtonDownEvent || event_id == UIElement::MouseLeftButtonUpEvent || event_id == UIElement::MouseRightButtonDownEvent || event_id == UIElement::MouseRightButtonUpEvent)
        ...
        else if (event_id == UIElement::KeyDownEvent || event_id == UIElement::KeyUpEvent)
...
#define SPEW_INPUT_LIST 0
    ...
    Runtime::Init(const char *platform_dir, RuntimeInitFlag flags, bool out_of_browser) {
        ...
        // Allow the user to override the flags via their environment
...
#elif PAL_WIN32_WINDOWING
        windowing_system = new MoonWindowingSystemWin32(out_of_browser);
#elif PAL_SDL2_WINDOWING
        windowing_system = new MoonWindowingSystemSDL2(out_of_browser);
#else
