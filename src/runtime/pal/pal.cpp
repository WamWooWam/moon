...
namespace Moonlight {
    ...
    void MoonWindowingSystem::SetWindowlessCtor(MoonWindowlessCtor ctor) {
        windowless_ctor = ctor;
    }

    MoonWindow *MoonWindowingSystem::CreateWindowless(int width, int height, PluginInstance *forPlugin) {
    ...
    bool MoonAppRecord::Equal(const MoonAppRecord *app) const {
        return !strcmp(app->uid, uid);
    }

    bool MoonAppRecord::Save(FILE *db) const {
    ...
    bool MoonAppRecordIterator::Fill() {
    ...
    bool MoonAppRecordIterator::EatWhiteSpace(bool lf) {
        char lwsp[4] = { ' ', '\t', '\n', '\0' };
    ...
    char MoonAppRecordIterator::NextToken() {
    ...
    char *MoonAppRecordIterator::ParseUid() {
        char *start, *str;
        GString *uid;
    ...
    char *MoonAppRecordIterator::ParseOrigin() {
        char *start, *str;
        GString *origin;
    ...
    time_t MoonAppRecordIterator::ParseMTime() {
        gint64 mtime;
        char *ptr;
    ...
    MoonAppRecord *MoonAppRecordIterator::Next() {
    ...
    MoonAppRecord *MoonAppDatabase::CreateAppRecord(const Uri *origin) {
    ...
    bool MoonAppDatabase::AddAppRecord(MoonAppRecord *record) {
        bool saved;
        FILE *db;
    ...
    bool MoonAppDatabase::SyncAppRecord(const MoonAppRecord *record) {
    ...
    bool MoonAppDatabase::RemoveAppRecord(const MoonAppRecord *record) {
        MoonAppRecordIterator *iter;
        MoonAppRecord *app;
        bool error = false;
        FILE *orig, *db;
    ...
    MoonAppRecord *MoonAppDatabase::GetAppRecordByOrigin(const Uri *origin) {
        MoonAppRecordIterator *iter;
        MoonAppRecord *app;
        FILE *db;
    ...
    MoonAppRecord *MoonAppDatabase::GetAppRecordByUid(const char *uid) {
        MoonAppRecordIterator *iter;
        MoonAppRecord *app;
        FILE *db;
    ...
    bool MoonInstallerService::InitDatabase() {
    ...
    void MoonInstallerService::CloseDownloader(bool abort) {
        if (request) {
            if (abort)
                request->Abort();
    ...
    MoonAppRecord *MoonInstallerService::CreateAppRecord(const Uri *origin) {
        if (!InitDatabase())
            return NULL;
    ...
    MoonAppRecord *MoonInstallerService::GetAppRecord(Deployment *deployment) {
    ...
    MoonAppRecord *MoonInstallerService::GetAppRecord(const char *uid) {
        if (!InitDatabase())
            return NULL;
    ...
    void MoonInstallerService::downloader_stopped(EventObject *sender, EventArgs *args, gpointer user_data) {
    ...
    void MoonInstallerService::UpdaterFailed(const char *msg) {
    ...
    void MoonInstallerService::CheckAndDownloadUpdateAsync(Deployment *deployment, UpdateCompletedCallback completed, gpointer user_data) {
        char mtime[128];
        struct tm tm;
    ...
    bool MoonInstallerService::IsRunningOutOfBrowser(Deployment *deployment) {
        const Uri *xap_uri = deployment->GetXapLocation();
        const char *base_dir = GetBaseInstallDir();
        const char *xap_path;
        size_t n;
    ...
    bool MoonInstallerService::CheckInstalled(Deployment *deployment) {
        MoonAppRecord *app;
    ...
    bool MoonInstallerService::Uninstall(Deployment *deployment) {
    ...
    guint32 MoonFontService::GetCharIndex(FT_Face face, gunichar unichar) {
...
int __stdcall _CreateSymbolicLinkA(
    ...
    const wchar_t *wSymlinkFileName = (const wchar_t *)g_utf8_to_utf16(lpSymlinkFileName, -1, NULL, NULL, NULL);
    const wchar_t *wTargetFileName = (const wchar_t *)g_utf8_to_utf16(lpTargetFileName, -1, NULL, NULL, NULL);

    int result = CreateSymbolicLinkW(wSymlinkFileName, wTargetFileName, dwFlags);

    g_free((void *)wSymlinkFileName);
    g_free((void *)wTargetFileName);
