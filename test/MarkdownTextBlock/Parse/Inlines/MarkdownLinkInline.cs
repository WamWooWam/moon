    ...
    internal class MarkdownLinkInline : MarkdownInline, IInlineContainer, ILinkElement
    {
        ...
        internal static InlineParseResult Parse(string markdown, int start, int maxEnd)
        {
            ...
            if (markdown[pos] == '(')
            {
                ...
                int tooltipStart = ParseHelpers.IndexOf(markdown, " \"", linkOpen, linkClose - 1);
                ...
                {
                    Inlines = ParseHelpers.ParseInlineChildren(markdown, linkTextOpen, linkTextClose, ignoreLinks: true),
                    Url = url.Replace(" ", "%20"),
                    Tooltip = tooltip
                };
            ...
            {
                // Find the ']' character.
                int linkClose = ParseHelpers.IndexOf(markdown, ']', pos + 1, maxEnd);
                if (linkClose == -1)
                {
                    return null;
                }
        ...
        public void ResolveReference(MarkdownDocument document)
        {
            ...
            LinkReferenceBlock reference = document.LookUpReference(ReferenceId);
            if (reference == null)
            {
                return;
            }
        ...
        private static bool IsUrlValid(string url)
        {
            ...
            foreach (string scheme in HyperlinkInline.KnownSchemes)
            {
