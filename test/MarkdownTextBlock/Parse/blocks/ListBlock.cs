...
namespace System.Windows.Controls.Markdown.Parse
{
    ...
    internal class ListBlock : MarkdownBlock
    {
        ...
        internal static ListBlock Parse(string markdown, int start, int maxEnd, int quoteDepth, out int actualEnd)
        {
            ...
            foreach (ParseHelpers.LineInfo lineInfo in ParseHelpers.ParseLines(markdown, start, maxEnd, quoteDepth))
            {
                // Is this line blank?
                ...
                foreach (MarkdownBlock block in listItem.Blocks)
                {
                    if (block is ListItemBuilder)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                Inlines = ParseHelpers.ParseInlineChildren(blockText, 0, blockText.Length)
                            };
