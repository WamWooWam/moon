...
namespace System.Windows.Controls.Markdown.Parse
{
    ...
    internal class HeaderBlock : MarkdownBlock
    {
        ...
        internal static HeaderBlock ParseHashPrefixedHeader(string markdown, int start, int end)
        {
            ...
            if (result.HeaderLevel == 0 || (pos < end && !char.IsWhiteSpace(markdown[pos])))
            {
                return null;
            }
            ...
            result.Inlines = ParseHelpers.ParseInlineChildren(markdown, pos, end);
            return result;
        }
        ...
        internal static HeaderBlock ParseUnderlineStyleHeader(string markdown, int firstLineStart, int firstLineEnd, int secondLineStart, int secondLineEnd)
        {
            ...
            HeaderBlock result = new HeaderBlock
            {
                ...
                // Parse the inline content.
                Inlines = ParseHelpers.ParseInlineChildren(markdown, firstLineStart, firstLineEnd)
            };
            return result;
        }
